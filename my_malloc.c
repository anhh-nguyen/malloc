...
static metadata_t *find_right(metadata_t *freed_block) { //!!!
    ...
    // if current is null and current.next is null
    if (!curr) {
        return NULL;
    }
    ...
    while (curr && ((uintptr_t) freed_block != (uintptr_t) curr)) { //iterate untill get to the freed block in the list
        curr = curr->next;
    }
    // if there is a right block is that exactly at the address of the current block + TMS
    if ((uintptr_t) ((uint8_t*) (curr + 1) + curr->size) == (uintptr_t) curr->next) { 
        return curr->next;
    }
    return NULL;
}
...
static metadata_t *find_left(metadata_t *freed_block) {
    ...
    while (curr && ((uintptr_t) freed_block > (uintptr_t) curr)) { //loops until curr is to the left of free blcok
        if ((uintptr_t) ((uint8_t*) (curr + 1) + curr->size) == (uintptr_t) freed_block) { //if the right of curr is the free block
            return curr;
        }
        curr = curr->next;
    }
...
static metadata_t *split_block(metadata_t *block, size_t size) {
    ...
    // 2. Using the new block's total size with the address and size of the old block, compute the address of the start of the new block.
    metadata_t *newBlock = (metadata_t*)((uint8_t*)(block + 1) + (block->size - newBlockSize));
    // Shrink the size of the old/left block to account for the lost size. This block should stay in the freelist.
    block->size = block->size - newBlockSize;
    //  Set the size of the new/right block and return it. This block should not go in the freelist.
    newBlock->size = size;
    return newBlock;
}
...
static void add_to_addr_list(metadata_t *block) {
    ...
    // case that the block to be added has smaller address than head
    if ((uintptr_t) block < (uintptr_t) address_list) { //cast to uintptr_t to compare address
        block->next = address_list;
        address_list = block;
        return;
    }
    // loop to the where curr is smaller than the freed block address and next is bigger than the freed block address
    metadata_t *curr = address_list;
    metadata_t *next;
    while ((next = curr->next) && (uintptr_t) block > (uintptr_t) next) {
        curr = next;
    }
    // add the freed block in the middle
    metadata_t *nextBlockPointer = curr->next;
    curr->next = block;
    block->next = nextBlockPointer;
}
...
static void remove_from_addr_list(metadata_t *block) {
    ...
    if (!curr) {
        return;
    } else if (curr == block) {     //if the first node is the block, have address list point to next block
        address_list = curr->next;
    }

    metadata_t *next;
    while ((next = curr->next) && (uintptr_t) block > (uintptr_t) next) { //if address of block is greater than the next node, move to next node
        curr = next;
    }
    if (next == block) {    //if the next node is equal to the block, set next to next.next node
        curr->next = next->next;
    }
}
...
static metadata_t *find_best_fit(size_t size) {
    ...
    while (curr) {
        if (curr->size == size) {
            ...
            if (!bestFit) {
                bestFit = curr; // account case that bestFit is null and cannot compare
            } else if ( curr->size < bestFit->size) {
                bestFit = curr;
            }
        }
        curr = curr->next;
    ...
    if (bestFit) {
        if (bestFit->size < TOTAL_METADATA_SIZE + size  + 1) {
            return bestFit;
        }
...
void *my_malloc(size_t size) {
    ...
    if (size == 0) {    // if the requested is too small
        return NULL;
    }
    // find the first best fit
    metadata_t *bestFit = find_best_fit(size);
    // if there is no big enough block then call sbrk
    if (!bestFit) {
        metadata_t *newBlock = my_sbrk(SBRK_SIZE);
        if (newBlock == (metadata_t*)-1) { //return -1
            my_malloc_errno = OUT_OF_MEMORY;
            return NULL;
        }
        newBlock->size = SBRK_SIZE - TOTAL_METADATA_SIZE; //acount for TMS
        my_free(newBlock + 1); //jump over the MTS
        bestFit = find_best_fit(size);
    }
    remove_from_addr_list(bestFit);
    return bestFit + 1; //jump over the MTS
...
void my_free(void *ptr) {
    ...
    metadata_t *block = ptr; //cast to metadata_t to be able tp step over the TMS and find address
    if (block) {
        block = block - 1; // +1 is jumping over TMS, -1 is move back to include TMS
...
void *my_calloc(size_t nmemb, size_t size) {
    ...
    memset((metadata_t*)pointer, 0, nmemb*size); 
